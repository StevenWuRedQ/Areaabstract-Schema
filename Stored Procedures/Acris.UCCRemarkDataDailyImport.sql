...
-- Input tables:			acris.UCCRemark
--							acris.tfnUCCRemarkDataDaily
--							

-- Tables modified:			acris.UCCRemark
...
BEGIN
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			FROM[stage].[tfnUCCRemarkDataDaily]('U') a
			...
			EXEC Utilities.util.[CreateValuesFragementForAudit] 'AreaAbstractNew', 'UCCRemark', 'UniqueKey, Sequence, DateLastUpdated', @outStr OUTPUT, @cmdStr OUTPUT, 'acris'
			...
			SET @outStr = N' INSERT INTO dbo.ColumnTransactionCommitted' +
						...
						+ N' FROM  stage.tfnUCCRemarkDataDaily(''U'') R1'
						+ N' INNER JOIN acris.UCCRemark R2 ON R1.UniqueKey=R2.UniqueKey AND R1.Sequence=R2.Sequence'
						+ N' CROSS APPLY	( '
						+ @outStr + N') '
						+ N' C (COL, VAL1, VAL2)'
			...
			SET @outStr=''
			SET @cmdStr=''
			EXEC Utilities.util.[CreateSetFragementForUpdate] 'AreaAbstractNew', 'UCCRemark', 'UniqueKey, Sequence', @outStr OUTPUT, @cmdStr OUTPUT, 'acris'

			SET @outStr = N' UPDATE a '
						+ @outStr +
						+ N' FROM acris.UCCRemark a, stage.tfnUCCRemarkDataDaily(''U'') b'
