...
BEGIN
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			FROM[stage].[tfnUCCMasterDataDaily]('U') a
			...
			EXEC Utilities.util.[CreateValuesFragementForAudit] 'AreaAbstractNew', 'UCCMaster', 'UniqueKey, DateLastUpdated', @outStr OUTPUT, @cmdStr OUTPUT, 'acris'
			...
			SET @outStr = N' INSERT INTO dbo.ColumnTransactionCommitted' +
						...
						+ N' FROM  stage.tfnUCCMasterDataDaily(''U'') R1'
						+ N' INNER JOIN acris.UCCMaster R2 ON R1.UniqueKey=R2.UniqueKey'
						+ N' CROSS APPLY	( '
						+ @outStr + N') '
						+ N' C (COL, VAL1, VAL2)'
			...
			SET @outStr=''
			SET @cmdStr=''
			EXEC Utilities.util.[CreateSetFragementForUpdate] 'AreaAbstractNew', 'UCCMaster', 'UniqueKey', @outStr OUTPUT, @cmdStr OUTPUT, 'acris'

			SET @outStr = N' UPDATE a '
						+ @outStr +
						+ N' FROM acris.UCCMaster a, stage.tfnUCCMasterDataDaily(''U'') b'
